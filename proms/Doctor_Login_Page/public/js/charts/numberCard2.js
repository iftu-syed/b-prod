...
document.addEventListener("DOMContentLoaded", () => {
    waitForDropdownsToLoad(() => {
        ...
        const fetchNumberCard2Data = (department, siteName) => {
            const queryParams = new URLSearchParams({
                ...(department && { department }),
                ...(siteName && { siteName })
            }).toString();
        ...
        fetchNumberCard2Data(departmentDropdown.value, siteNameDropdown.value);

        // Add event listener to update number card 2 on department or site change
        departmentDropdown.addEventListener("change", () => {
            fetchNumberCard2Data(departmentDropdown.value, siteNameDropdown.value);
        });
