...
#     layout = {
#         "title": f'Mr_no : {mr_no} | {health_type.capitalize()} Health',
#         "xaxis": dict(
#             title='Timeline (Months)',
#             tickvals=list(range(1, len(months_since_initial) + 2)),  # Extend by 1 month
#             ticktext=["Baseline" if i == 1 else f"{i} month{'s' if i > 1 else ''}" for i in range(1, len(months_since_initial) + 2)],
...
#     if not os.path.exists(output_dir):
#         os.makedirs(output_dir)
#     fig.write_image(os.path.join(output_dir, f"{mr_no}_{health_type}_health.jpg"))
...
#this is the new code for all the graphs 
...
# # Function to generate the graph for physical health
# def generate_graph(mr_no, health_type):
#     responses = fetch_promis_responses(mr_no)
#     if not responses:
#         print(f"No PROMIS-10 data found for Mr_no: {mr_no}")
...
#     raw_scores_by_date = calculate_raw_scores(responses, health_type)  # Call the function to calculate scores
#     dates = sorted(raw_scores_by_date.keys())
#     scores = [raw_scores_by_date[date] for date in dates]
...
#             range=[0.5, len(months_since_initial) + 1.5]  # Ensure proper spacing
#         ),
#         "yaxis": dict(title='T-Score'),  # Update Y-axis title to reflect T-scores
#         "plot_bgcolor": 'rgba(0,0,0,0)',
#         "paper_bgcolor": 'rgba(255,255,255,1)',
#         "hovermode": 'closest'
#     }
...
# # Function to fetch PROMIS responses
# def fetch_promis_responses(mr_no):
#     document = collection.find_one({"Mr_no": mr_no}, {"PROMIS-10": 1})
#     if document and "PROMIS-10" in document:
#         return document["PROMIS-10"]
...
#         timestamp = response.get("timestamp")
#         date = datetime.strptime(timestamp[:10], "%Y-%m-%d").strftime("%Y-%m-%d")
#         recoded_response = recode_responses(response)
#         if health_type == 'physical':
#             raw_score = sum(recoded_response.get(q, 0) for q in PHYSICAL_HEALTH_QUESTIONS.keys())
#         else:
...
# Function to generate the graph for physical and mental health
def generate_graph(mr_no, health_type):
    responses = fetch_promis_responses(mr_no)
    if not responses:
        print(f"No PROMIS-10 data found for Mr_no: {mr_no}")
