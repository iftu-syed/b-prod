...
//       //This is the code that removes the hard code of the Speciality because, we focus is on the API object.
...
//     // If the current survey is the last one, mark as completed
...
//     // If the current survey is the last one, check the API array
//     if (currentSurveyIndex === customSurveyNames.length - 1) {
//       // If API is not empty, redirect to the 8080 port
//       if (apiSurvey && apiSurvey.length > 0) {
//         return res.redirect(`http://localhost:8080?mr_no=${Mr_no}&lang=${lang}`);
//       } else {
//         // Otherwise, mark the survey as completed and show the details page
//         await db1.collection('patient_data').updateOne(
//           { Mr_no },
//           { $set: { surveyStatus: 'Completed' } }
//         );
