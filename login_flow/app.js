  ...
  try {
    ...
    const executeBackgroundTasks = async (patientMrNo) => {
      try {
        ...
        await exec(`curl -X POST https://app.wehealthify.org/patientlogin/run-scripts \
        ...
        await Promise.all([
          exec(`curl -X POST https://app.wehealthify.org/doctorlogin/doctor-llama-script \
            -H "Content-Type: application/json" -d '{"mr_no": "${patientMrNo}"}'`),
          exec(`curl -X POST https://app.wehealthify.org/patientlogin/api_script \
            -H "Content-Type: application/json" -d '{"mr_no": "${patientMrNo}"}'`)
        ]);
            ...
            if (appointment.surveyStatus === "Not Completed" && Array.isArray(appointment.survey_name) && appointment.survey_name.includes(collectionName)) {
                ...
                if (updatedAppointmentToCheck && Array.isArray(updatedAppointmentToCheck.survey_name) && updatedAppointmentToCheck.survey_name.length > 0) {
                    ...
                    if (allSurveysInThisAppointmentDone) {
                        ...
                        // --- Start of Change ---
                        const completionTimeUpdatePath = `${appointmentFieldPathBase}.${i}.Completedsurveytime`;

                        await db1.collection('patient_data').updateOne(
                            { Mr_no: actualMrNo },
                            { 
                                $set: { 
                                    [appointmentStatusUpdatePath]: "Completed",
                                    [completionTimeUpdatePath]: currentSubmissionTimestamp.toISOString()
                                } 
                            }
                        );
                        console.log(` Appointment index ${i} marked 'Completed' with timestamp (MRN: ${actualMrNo}).`);
                        // --- End of Change ---
                    }
